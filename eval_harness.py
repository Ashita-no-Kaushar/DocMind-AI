...
QUERIES = [
    ...
    {
        "query": "money back rules",
        "expected": "refund_policy.txt",
        "feature": "synonym",
    },
    {"query": "30-day returns", "expected": "refund_policy.txt", "feature": "hyphen"},
    {
        "query": "refund policy batao",
        "expected": "refund_policy.txt",
        "feature": "hinglish",
    },
    {
        "query": "annual report revenue growth",
        "expected": "annual_report.txt",
        "feature": "title-aware",
    },
    {
        "query": "How to fetch a URL in Python?",
        "expected": "python_guide.txt",
        "feature": "basic",
    },
    {
        "query": "How many leave days per year?",
        "expected": "hr_policy.txt",
        "feature": "basic",
    },
    ...
    {
        "query": "refunded purchases",
        "expected": "refund_policy.txt",
        "feature": "stemming",
    },
    {
        "query": "tell me about the annual report",
        "expected": "annual_report.txt",
        "feature": "filler-filter",
    },
]
...
def _evaluate_retriever(retriever, query: str, expected: str | None):
    ...
    return {
        ...
        "latency": latency,
        "top_file": top_file,
        "top_score": round(float(top_score), 3) if nodes else 0.0,
        "num_nodes": len(nodes),
    }
...
def suite_ingestion(mock: bool):
    ...
    try:
        ...
        if (
            not getattr(Settings, "embed_model", None)
            or getattr(Settings.embed_model, "model_name", "") == "unknown"
        ):
    ...
    def check(name, fn):
        try:
            ...
            results.append(
                {"test": name, "pass": False, "detail": f"exception: {e}", "ms": 0}
            )
    ...
    def t_excluded():
        ...
        d = tempfile.mkdtemp()
        try:
            Path(d, "keep.txt").write_text(
                "hello world content for testing keep file", encoding="utf-8"
            )
    ...
    def t_title():
        ...
        n = TextNode(
            text="body content here", metadata={"file_name": "annual_report.md"}
        )
        ...
        try:
            ...
            try:
                ...
                if "docx2txt" in str(e):
                    ...
                    Path(d, "d.txt").write_text(
                        DOCS["python_guide.txt"], encoding="utf-8"
                    )
                    docs = load_documents(d)
                    docx_ok = False
                else:
            ...
            return (
                ok,
                f"docs={len(docs)} docx={'yes' if docx_ok else 'skip-no-docx2txt'}",
            )
        finally:
            shutil.rmtree(d, ignore_errors=True)
    ...
    def t_min_chars():
        from utils.llama_index import MIN_CHUNK_CHARS, create_index, load_documents

        d = tempfile.mkdtemp()
        try:
            Path(d, "tiny.txt").write_text("hi", encoding="utf-8")
            Path(d, "ok.txt").write_text(
                "This is a sufficiently long document content for chunking tests. " * 5,
                encoding="utf-8",
            )
            docs = load_documents(d)
            try:
    ...
    def t_helpers():
        ...
        try:
            ...
            return (
                ok1 and ok2 and ok3
            ), f"owner/repo={ok1} url={ok2} reject_gitlab={ok3}"
        except Exception as e:
            return False, str(e)
...
def suite_retrieval(
    mock: bool, endpoint="http://localhost:11434", model="nomic-embed-text:latest"
):
    ...
    try:
        from utils.llama_index import HybridRetriever, build_hybrid_retriever
        ...
        hybrid = build_hybrid_retriever(
            vector_retriever, index, top_k=3, similarity_cutoff=0.3
        )
        ...
        for item in QUERIES:
            ...
            print(
                f"  {h_mark if hr['hit'] else 'FAIL'}  [{item['feature']:12s}] {q[:42]:42s}  plain:{p_mark} hybrid:{h_mark} ({pr['top_file'] or '-'} -> {hr['top_file'] or '-'})"
            )
            results.append(
                {
                    "query": q,
                    ...
                    "plain_hit": pr["hit"],
                    "hybrid_hit": hr["hit"],
                    "plain_top": pr["top_file"],
                    "hybrid_top": hr["top_file"],
                    "plain_score": pr["top_score"],
                    "hybrid_score": hr["top_score"],
                    "plain_latency": round(pr["latency"], 4),
                    "hybrid_latency": round(hr["latency"], 4),
                }
            )
        ...
        def hr(rows, key):
            return sum(1 for r in rows if r[key]) / len(rows) if rows else 0
        ...
        summary = {
            ...
            "plain_avg_ms": (
                round(sum(r["plain_latency"] for r in results) / len(results) * 1000, 1)
                if results
                else 0
            ),
            "hybrid_avg_ms": (
                round(
                    sum(r["hybrid_latency"] for r in results) / len(results) * 1000, 1
                )
                if results
                else 0
            ),
        ...
        print(
            f"  Summary: hit plain {summary['plain_hit_rate']:.0%} hybrid {summary['hybrid_hit_rate']:.0%} | reject plain {summary['plain_rejection']:.0%} hybrid {summary['hybrid_rejection']:.0%} | latency plain {summary['plain_avg_ms']}ms hybrid {summary['hybrid_avg_ms']}ms"
        )
...
def suite_generation(
    ...
    def t_template():
        ...
        tmpl = (
            ...
            if hasattr(TEXT_QA_TEMPLATE, "template")
            else str(TEXT_QA_TEMPLATE)
        )
        ok = (
            "ONLY the context" in tmpl
            and "could not find" in tmpl.lower()
            and "{context_str}" in tmpl
        )
    ...
    def t_tone():
        from components.chatbox import ANSWER_STYLE_OPTIONS, _style_to_prompt
    ...
    def t_multiturn():
        ...
        node = TextNode(
            text="Refund 30 days.", metadata={"file_name": "refund_policy.txt"}
        )
    ...
    def t_e2e_real():
        ...
        try:
            ...
            models = [
                m.get("model") or m.get("name") for m in client.list().get("models", [])
            ]
            ...
            d = tempfile.mkdtemp()
            try:
                Path(d, "refund_policy.txt").write_text(
                    DOCS["refund_policy.txt"], encoding="utf-8"
                )
                ...
                for _ in range(3):
                    answers.append(
                        "".join(
                            context_chat("What is the refund policy?", query_engine=qe)
                        )
                    )
                successful = sum(
                    "30" in answer
                    and ("day" in answer.lower() or "refund" in answer.lower())
                ...
                return (
                    ok,
                    f"successful={successful}/3 lengths={[len(answer) for answer in answers]} preview={preview!r}",
                )
            finally:
                shutil.rmtree(d, ignore_errors=True)
        except Exception as e:
...
def suite_performance(
    ...
    def t_retrieval_latency():
        ...
        try:
            ...
            t0 = time.perf_counter()
            hy.retrieve("refund policy")
            h_ms = (time.perf_counter() - t0) * 1000
            t0 = time.perf_counter()
            vr.retrieve("refund policy")
    ...
    def t_eco():
        ...
        from utils.ollama import _embed_batch_size, _num_predict

        st.session_state["eco_mode"] = False
        normal_ctx = _context_char_budget()
        normal_pred = _num_predict()
        normal_batch = _embed_batch_size()
        st.session_state["eco_mode"] = True
        eco_ctx = _context_char_budget()
        eco_pred = _num_predict()
        eco_batch = _embed_batch_size()
        st.session_state["eco_mode"] = False
        ok = (
            eco_ctx < normal_ctx and eco_pred < normal_pred and eco_batch < normal_batch
        )
        return (
            ok,
            f"ctx {normal_ctx}->{eco_ctx} pred {normal_pred}->{eco_pred} batch {normal_batch}->{eco_batch}",
        )
    ...
    def t_batch_oom():
        ...
        emb = OllamaEmbedding(
            model_name="nomic-embed-text:latest", base_url="http://localhost:11434"
        )
    ...
    def t_empty_docs():
        ...
        try:
            ...
            try:
                ...
                detail = str(e)
                cause = e.__cause__
                safe_rejection = "No usable" in detail or (
                    "Index creation failed safely" in detail
                    and cause is not None
                    and "No usable" in str(cause)
                )
                return safe_rejection, detail[:80]
        finally:
            shutil.rmtree(d, ignore_errors=True)
...
def suite_architecture(mock: bool):
    ...
    def t_browser_settings():
        ...
        secrets_excluded = not {"openai_api_key", "r2r_api_key"} & set(
            PERSISTED_SETTING_TYPES
        )
        ok = (
        ...
        return (
            ok,
            f"settings={len(PERSISTED_SETTING_TYPES)} secrets_excluded={secrets_excluded}",
        )
    ...
    def t_ollama_helpers():
        ...
        msgs = [
            ChatMessage(role=MessageRole.USER, content="a" * 4000),
...
def _write_outputs(all_results, overall, out_dir="."):
    ...
    for name in ALL_SUITES:
        ...
        r = all_results[name]
        md.append(
            f"| {name} | {r.get('passed',0)}/{r.get('scored',0)} | {r.get('skipped',0)} | **{r.get('score',0):.1%}** | {descs.get(name,'')} |"
        )
    md.append(
        f"| **Overall** | **{overall['passed']}/{overall['scored']}** | **{overall['skipped']}** | **{overall['overall_score']:.1%}** | Raw scored-check rate |"
    )
    md.append("")
    ...
    if "retrieval" in all_results:
        ...
        if summ:
            md.append(
                f"_Embedding: `{summ.get('embedding','')}` — hit = expected doc in top-3, correct rejection = no-match returns 0 nodes_"
            )
            md.append("")
            md.append("| Metric | Plain RAG | DocMind Hybrid |")
            md.append("|---|---:|---:|")
            md.append(
                f"| Factual hit rate | {summ.get('plain_hit_rate',0):.0%} | **{summ.get('hybrid_hit_rate',0):.0%}** |"
            )
            md.append(
                f"| Correct rejection | {summ.get('plain_rejection',0):.0%} | **{summ.get('hybrid_rejection',0):.0%}** |"
            )
            md.append(
                f"| Avg latency | {summ.get('plain_avg_ms',0):.0f} ms | {summ.get('hybrid_avg_ms',0):.0f} ms |"
            )
            md.append("")
        ...
        if results:
            ...
            for i, r in enumerate(results, 1):
                ...
                md.append(
                    f"| {i} | {r['query']} | {exp} | {r['feature']} | {p} | {h} | {r['plain_top'] or '—'} | {r['hybrid_top'] or '—'} |"
                )
            md.append("")

    # Per-suite breakdowns
    for name in ALL_SUITES:
        if name not in all_results or name == "retrieval":
            continue
        r = all_results[name]
        md.append(
            f"## Suite: {name} ({r.get('passed',0)}/{r.get('scored',0)} scored, {r.get('skipped',0)} skipped — {r.get('score',0):.1%})"
        )
        md.append("")
        ...
        for t in r.get("results", []):
            status = (
                "SKIP"
                if t.get("pass") is None
                else ("PASS" if t.get("pass") else "FAIL")
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--endpoint", default="http://localhost:11434", help="Ollama endpoint"
    )
    parser.add_argument(
        "--model", default="nomic-embed-text:latest", help="Embedding model"
    )
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Use hash embeddings + mocked LLM (no Ollama)",
    )
    ...
    all_results, overall = run_full_eval(
        endpoint=args.endpoint, model=args.model, use_mock=use_mock, suites=suites
    )
